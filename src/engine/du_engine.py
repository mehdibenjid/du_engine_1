...
def build_keywords(row) -> str:
    """Concatenates Header and Body keywords into a single string."""
    vals = []
...
def sanitize_text(text: str) -> str:
    """Cleans text by removing specific 'bad' tokens and normalizing whitespace."""
...
def build_dense_text(row: pd.Series, max_chars: int = 1200) -> str:
    """
    Constructs the string to be sent to the embedding model.
    
    Composition: Title + Narrative + Reason + Action + System.
    Condition (Patch 3): Includes Intent/Category only if the Narrative is empty 
    to prevent category labels from skewing semantic matching when rich text exists.
    """
    parts = []

    title = sanitize_text(row.get("Title", ""))
    text = row.get("Text", "")
    narrative = strip_id_noise_for_dense(text, max_ids_to_keep=6)

    if max_chars and len(narrative) > max_chars:
    ...
    for k in ["Reason", "Action", "System", "Rubric"]:
        v = sanitize_text(row.get(k, ""))
        if valid_feature(v):
            parts.append(f"{k}: {v}")

    # Patch 3 (only if no narrative)
    if not narrative:
        for k in ["Intent", "Category"]:
            v = sanitize_text(row.get(k, ""))
            if valid_feature(v):
                parts.append(f"{k}: {v}")

    return " | ".join(parts) if parts else "Document technique"
...
def build_session(max_workers: int) -> requests.Session:
    """Creates a Requests session with a connection pool sized for concurrency."""
    s = requests.Session()
...
# -----------------------------------------------------------------------------
def _embed_call(session: requests.Session, text: str, token: str, timeout: int = 30) -> Tuple[np.ndarray, str]:
    """
    Low-level function to call the embedding API.
    ...
    payload = {"inputText": text, "dimensions": DIMS, "normalize": True}

    resp = session.post(API_BASE_URL, json=payload, headers=headers, verify=False, timeout=timeout)
...
def embed_one_safe(
    ...
    Returns:
        ...
        If successful, vector is np.array and error_message is None.
    """
    ...
    for attempt in range(retries):
        try:
            vec, token = _embed_call(session, text, token, timeout=timeout)
            if vec is None or vec.shape[0] != DIMS:
                return None, token, "bad_vector"
            return vec, token, None
        except Exception as e:
            last_err = f"exc_{type(e).__name__}"
            time.sleep(0.8 * (attempt + 1)) # Linear/Exponential backoff
            continue
...
class DUEngine:
    ...
    def __init__(self, artifacts_dir: str = "artifacts"):
        ...
        logger.info("Loading vectors matrix for MMR...")
    ...
    def retrieve_batch(
        ...
        logger.info(
            "Embedding phase summary | unique=%d success=%d fail=%d rate=%.2f%% "
            "total_time=%.2fs avg=%.3fs workers=%d",
            total, success, fail, success_rate, t_embed_total, avg_per, workers,
        )
        if success_rate < 98.0:
            logger.warning("embedding success_rate=%.2f%% < 98%%", success_rate)
            if fail:
                for txt, err in list(text_to_err.items())[:5]:
                    logger.warning("  err=%s | text_preview=%r", err, txt[:120])
